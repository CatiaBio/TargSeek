...
def search_pdb_via_uniprot(gene_name, max_structures=3):
    """
    Alternative search using UniProt to find PDB structures
    """
    try:
        ...
        if response.status_code != 200:
            logging.warning(f"UniProt search failed for {gene_name}: status {response.status_code}")
            return []
...
def download_3d_structures_for_gene(gene_name, structures_base_dir, organisms=None, max_structures=3):
    """
    ...
    # Search for PDB structures (already sorted by score)
    pdb_ids = search_pdb_structures(gene_name, organisms, max_structures)
...
def process_proteins_list(proteins_file, structures_base_dir):
    """
...
def main():
    ...
    summary = process_proteins_list(proteins_file, structures_dir)
