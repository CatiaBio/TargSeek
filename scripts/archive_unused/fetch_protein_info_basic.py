#!/usr/bin/env python3
"""
Basic Comprehensive Protein Information Fetcher
===============================================
...
Uses no field specification to get all available data.
"""
...
def fetch_complete_protein_info(gene_name, retries=3, delay=1):
    ...
    # Get all data without specifying fields
    params = {
        'query': query,
        'format': 'json',
    ...
    for attempt in range(retries):
        try:
            ...
            if data['results']:
                ...
                protein_info['retrieval_date'] = time.strftime('%Y-%m-%d')
...
def extract_comments(comments, protein_info):
    ...
    for comment in comments:
        ...
        text_value = comment.get('texts', [{}])[0].get('value', '')
...
def extract_go_terms(go_terms, protein_info):
    ...
    protein_info['go_terms_all'] = '; '.join(all_go)
...
def extract_features(features, protein_info):
    ...
    for feature in features:
        ...
        if feature_type == 'DOMAIN':
            ...
            glycosylation.append(description)
        elif feature_type == 'LIPID':
            lipidation.append(description)
        elif feature_type == 'MOD_RES':
            modifications.append(description)
        elif feature_type in ['HELIX', 'STRAND', 'TURN', 'COILED']:
    ...
    protein_info['glycosylation'] = '; '.join(filter(None, glycosylation))
    protein_info['lipidation'] = '; '.join(filter(None, lipidation))
...
def extract_cross_references(cross_refs, protein_info):
    ...
    protein_info['go_database'] = '; '.join(go_database)
    protein_info['interpro'] = '; '.join(interpro)
    protein_info['pfam'] = '; '.join(pfam)
    protein_info['smart'] = '; '.join(smart)
...
def extract_protein_families(protein_desc, protein_info):
    ...
    # Also check in alternativeNames for family info
    for alt_name in protein_desc.get('alternativeNames', []):
        if 'fullName' in alt_name and 'family' in alt_name['fullName'].get('value', '').lower():
            families.append(alt_name['fullName']['value'])
...
def process_gene_list(gene_list, output_dir):
    ...
    for idx, gene_name in enumerate(gene_list):
        ...
        if protein_info:
            ...
            print(f"  GO Terms: {len(protein_info['go_terms_all'].split(';')) if protein_info['go_terms_all'] else 0}")
    ...
    if all_results:
        ...
        go_count = sum(1 for r in all_results if r['go_terms_all'])
