...
def main():
    ...
    # Create output directory for tracking files
    output_tracking_dir = Path(f"results/{analysis}_{paramset}")
    output_tracking_dir.mkdir(parents=True, exist_ok=True)
    
    # Write selected_3d_fasta.tsv (group-specific)
    if all_selected_3d:
        tsv_path = output_tracking_dir / f"selected_3d_fasta_gram_{group}.tsv"
    ...
    if all_3d_paths:
        paths_file = output_tracking_dir / f"selected_3d_paths_gram_{group}.txt"
