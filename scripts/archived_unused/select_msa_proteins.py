...
def find_available_sequences(gene_name, target_species, protein_sequences_dir="data/protein_sequences"):
    """
    Find available FASTA sequences for a gene from shared data directory
    
    Args:
        gene_name: Name of the gene
        target_species: List of species to look for
        protein_sequences_dir: Base directory for protein sequences
    
    Returns:
        dict: {species: fasta_file_path}
    """
    gene_fasta_dir = Path(protein_sequences_dir) / gene_name
    available_sequences = {}
    
    if not gene_fasta_dir.exists():
        logging.warning(f"No sequences found for {gene_name} - directory not found: {gene_fasta_dir}")
        return {}
...
def find_available_structures(gene_name, target_species, protein_structures_dir="data/protein_structures"):
    """
    ...
    Args:
        gene_name: Name of the gene
        target_species: List of species to look for
        protein_structures_dir: Base directory for protein structures
    
    Returns:
        dict: {species: pdb_file_path}
    """
    gene_structures_dir = Path(protein_structures_dir) / gene_name
    available_structures = {}
    
    if not gene_structures_dir.exists():
        logging.debug(f"No structures directory found for gene {gene_name}: {gene_structures_dir}")
        return {}
...
def create_3d_structure_filelist(gene_name, gene_msa_dir, sequence_references, protein_structures_dir="data/protein_structures"):
    """
    Create a filelist for 3D structures from the protein structures directory
    ...
    structures_dir = Path(protein_structures_dir) / gene_name
