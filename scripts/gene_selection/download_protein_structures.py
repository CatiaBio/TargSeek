...
def search_pdb_via_uniprot_bacterial(gene_name, max_structures=10, gene_aliases=None, cache=None):
    """
...
def _search_uniprot_bacterial_single_gene(gene_name, max_structures):
    ...
    try:
        ...
        if bacterial_pdb_ids:
            limited_bacterial = bacterial_pdb_ids[:max_structures]
            logging.info(f"Found {len(bacterial_pdb_ids)} bacterial structures for {gene_name}, using top {len(limited_bacterial)}")
            return limited_bacterial
        elif other_pdb_ids:
            limited_others = other_pdb_ids[:max_structures]
            logging.info(f"No bacterial structures found for {gene_name}, using {len(limited_others)} non-bacterial structures")
            return limited_others
        else:
            return []
...
def download_3d_structures_for_gene(gene_name, structures_base_dir, organisms=None, max_structures=10, gene_aliases=None, cache=None, include_computed_models=True):
    """
    ...
    # Some genes might have less than max_structures available
...
def main():
    ...
    # Update process_proteins_list to pass max_structures
