...
with open(snakemake.input.all_species, 'r') as f:
    all_species_list = [line.strip() for line in f if line.strip() and not line.strip().startswith('#')]
...
if len(all_species_list) == 0:
    print("ERROR: No species found in input file!")
    exit(1)
...
cache_hit_rate = (cached_queries / total_queries_needed) * 100 if total_queries_needed > 0 else 0
