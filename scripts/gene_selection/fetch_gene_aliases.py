...
def main():
    ...
    # Write results to TSV file with new format
    ...
    for gene in sorted(final_aliases.keys()):
        if final_aliases[gene]:
            ...
            tsv_data.append({'gene': gene, 'alias_list': aliases_str})
        else:
            tsv_data.append({'gene': gene, 'alias_list': ''})
    ...
    with open(metadata_file, 'w') as f:
        f.write("# Gene Aliases/Synonyms (Consolidated) - Metadata\n")
        f.write("# TSV Format: gene\talias_list (comma-separated aliases)\n")
    ...
    logging.info(f"\nResults saved to:")
    logging.info(f"  Aliases TSV: {output_path}")
